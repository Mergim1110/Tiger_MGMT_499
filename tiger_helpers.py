...
# pkg -> import name (when they differ)
_need = [
    ("pandas", "pandas"),
    ("numpy", "numpy"),
    ...
    ("plotly", "plotly"),
    ("geopandas", "geopandas"),
    ("pyogrio", "pyogrio"),  # lightweight vector IO backend for GeoPandas
]

for pkg, mod in _need:
    try:
        __import__(mod)
    except ImportError:
        _pip(pkg)

# Optional: set Plotly renderer if available
try:
